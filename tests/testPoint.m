classdef testPoint < matlab.unittest.TestCase
	...
	methods (TestMethodSetup)
		...
		function createTreeFrame(testCase)
			...
			testCase.fr2a = Frame("Frame2a", ...
				[2, 0, 0], [0, 0, 0], "321", testCase.fr1);
			testCase.fr2b = Frame("Frame2b", ...
				[0, 2, 0], [0, 0, 0], "321", testCase.fr1);
			testCase.fr2c = Frame("Frame2c", ...
				[0, 0, 2], [0, 0, 0], "321", testCase.fr1);

			testCase.fr3a = Frame("Frame3a", ...
				[0, 0, 0], [0, 0, pi/2], "321", testCase.fr2a);
			testCase.fr3b = Frame("Frame3b", ...
				[0, 0, 0], [0, pi/2, 0], "321", testCase.fr2b);
			testCase.fr3c = Frame("Frame3c", ...
				[0, 0, 0], [pi/2, 0, 0], "321", testCase.fr2c);
	...
	methods (Test)
		...
		function testSetPointCoords(testCase)
			p = Point(rand(1,3), testCase.world);
			expCoords1 = [1; 2; 3];
			expCoords2 = [4; 5; 6];
		...
		function testGetPointRadialVector(testCase)
			...
			testCase.verifyEqual(p.radius.mag, norm(expCoords));
		...
		function testViewIn(testCase)
			...
			testCase.verifyEqual(p1.viewIn(testCase.fr1), [10;10;10]);
			testCase.verifyEqual(p1.viewIn(testCase.world), [11;11;11]);
			testCase.verifyEqual(p1.viewIn(testCase.fr2a), [8;10;10]);
			testCase.verifyEqual(p1.viewIn(testCase.fr2b), [10;8;10]);
			testCase.verifyEqual(p1.viewIn(testCase.fr2c), [10;10;8]);
			testCase.verifyEqual(p1.viewIn(testCase.fr3a), [8;10;-10]);
			testCase.verifyEqual(p1.viewIn(testCase.fr3b), [-10;8;10]);
			testCase.verifyEqual(p1.viewIn(testCase.fr3c), [10;-10;8]);

			p3c = Point(coords, testCase.fr3c);
			testCase.verifyEqual(p3c.viewIn(testCase.fr2c), [-10;10;10]);
			testCase.verifyEqual(p3c.viewIn(testCase.world), [-9;11;13]);
			testCase.verifyEqual(p3c.viewIn(testCase.fr3a), [-12;12;-10]);
		end
		...
		function testTranslatePoint(testCase)
			...
			testCase.verifyEqual(p1.translate(v1).coords, expCoords1);
			testCase.verifyEqual(p1.translate(v2).coords, expCoords2);
			testCase.verifyEqual(p1.translate(v3).coords, expCoords3);
			testCase.verifyEqual(p1.translate(v4).coords, expCoords4);
		...
		function testRotatePoint(testCase)
			...
			testCase.verifyEqual(p1.rotate(r1,p0).coords, [0; 1; 0]);
			testCase.verifyEqual(p1.rotate(r2,p0).coords, [0; 1; 0]);
			testCase.verifyEqual(p1.rotate(r1,p0).coords, [-1; 0; 0]);
			testCase.verifyEqual(p1.rotate(r2,p0).coords, [0; 0; 1]);
			testCase.verifyEqual(p1.rotate(r3,p0).coords, [0; 1; 0]);
			testCase.verifyEqual( ...
				p1.rotate(r4,p0).coords, [sqrt(2); sqrt(2); 0]/2, ...
				AbsTol=1e-12);
			testCase.verifyEqual( ...
				p1.rotate(r4,p0).coords, [1; 0; 0], ...
				AbsTol=1e-12);
			testCase.verifyError( ...
